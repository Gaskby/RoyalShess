...
lista de reglas: **15 lecciones** en **4 capítulos** que se abren en cadena,
donde mueve el jugador y no se avanza hasta que hace lo que se le pide.

1. **Lo básico** — mover sin turnos · la energía y esperar · comer y el
   reembolso · el reloj y los Puntos.
2. **El rey** — jaque con su cuenta atrás · rey acorralado (tapar la línea) ·
   ganar comiéndote al rey tras el margen · enroque.
3. **Trucos de pieza** — racha del peón · descuento del caballo · la dama que
   se abarata · los carriles de torre y su peaje.
4. **Jugadas finas** — recaptura gratis · coronación · **duelo final** contra
   la máquina con las dos energías corriendo.

Los consejos los da **Chessy** (`public/coach.js`), el peón de píxeles que
escribe letra a letra, mueve la boca mientras habla y cambia de cara según lo
que pasa (señala el tablero, se asusta en jaque, celebra al superar, pone cara
de «eso no» si intentas algo imposible). Se edita como los emotes: el cuerpo es
un dibujo ASCII y los rasgos son rectángulos de píxeles; los colores salen de
la paleta del tema, así que cambia de color con el tablero.
...
el servidor —incluidas las finas: racha, descuentos, peajes, cupón de recaptura
y enroque—, así que al cambiar un número en la hoja de configuración el tutorial
enseña el nuevo sin tocarlo. En las lecciones que lo necesitan, **cada destino
muestra lo que cuesta ir ahí**: es la única forma de ver un peaje o un descuento.
El progreso se guarda en `localStorage` (`rs-learn`) y los textos están en
`i18n.js` bajo `learn.`.

La lista de reglas escritas de siempre sigue ahí, a un botón del tutorial
(**Reglas y trucos**), ya solo como chuleta de consulta.
...
└─ public/
   ...
   ├─ coach.js      Chessy: el peón de píxeles del tutorial y los emotes
...
- **Emotes:** 6 burlas bajo el tablero, en partidas online contra personas.
  Sin chat libre: nada que moderar. El servidor limita la frecuencia
  (1 cada 1.2 s) y solo recibe el índice, nunca texto.
  ...
  Quien vacila es **Chessy**, el peón de píxeles que enseña en el tutorial:
  mismo bicho, misma paleta (cuelga de la de `coach.js`), y sale **vestido
  del color de su bando**, así que se sabe quién se está riendo sin leer
  nada. Se dibuja en **`public/emotes.js`** (editable): el cuerpo es un
  dibujo ASCII y cada gesto son rectángulos de píxeles. Regla de oro al
  añadir uno: a 44 px un brazo pegado al cuerpo no se ve, así que los gestos
  van **fuera de la silueta** y con 2 píxeles de grosor mínimo.
  Están **animadas** (los movimientos viven en `style.css`, busca «animación
  de los emotes») y van a saltos (`steps`), que es lo que pide un sprite:
  se mueven al recibir un emote y al pasar el ratón por un botón, y están
  quietas el resto del tiempo para no distraer. Por eso toda animación
  empieza y acaba en la posición neutra: ese fotograma 0 es la cara parada
  del botón.
